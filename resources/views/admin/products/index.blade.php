                ...
                <table class="min-w-full responsive-table">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 md:divide-y-0">
                        @forelse ($products as $product)
                            <tr>
                                <td data-label="Nama Produk" class="px-6 py-4 whitespace-nowrap">{{ $product->name }}</td>
                                <td data-label="Deskripsi" class="px-6 py-4 text-sm text-gray-500">{{ $product->description ?? '-' }}</td>
                                <td data-label="Aksi" class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                                    <a href="{{ route('admin.products.edit', $product) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    <form class="inline-block" action="{{ route('admin.products.destroy', $product) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus produk ini?');">
                                        @csrf
                                        @method('DELETE')
